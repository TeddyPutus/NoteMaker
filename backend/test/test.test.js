...
describe("Testing User routes", () => {
    describe("Create a user", () => {
        describe("With valid values", () => {
            it("Returns 200 status", async () => {
                const { statusCode } = await request(app).post("/users").send({  email:"teddyputus1@gmail.com", password:"thisisapassword", username:"Tedernator" });
        ...
        describe("With invalid values, returns 400", () => {
            it("Returns 400 when username not unique", async () => {
                const { statusCode } = await request(app).post("/users").send({  email:"teddyputus2@gmail.com", password:"thisisapassword", username:"Tedernator" });
                expect(statusCode).toBe(400);
            })
            it("Returns 400 when username has spaces", async () => {
                const { statusCode } = await request(app).post("/users").send({  email:"teddyputus2@gmail.com", password:"thisisapassword", username:"Teder nator2" });
                expect(statusCode).toBe(400);
            })
            ...
            it("Returns 400 when email not unique", async () => {
                const { statusCode } = await request(app).post("/users").send({  email:"teddyputus1@gmail.com", password:"thisisapassword", username:"Tedernator2" });
                expect(statusCode).toBe(400);
            })
            it("Returns 400 when email not an email address", async () => {
                const { statusCode } = await request(app).post("/users").send({  email:"teddyputus1.com", password:"thisisapassword", username:"Tedernator2" });
                expect(statusCode).toBe(400);
            })
            it("Returns 400 when password too short", async () => {
                const { statusCode } = await request(app).post("/users").send({  email:"teddyputus2.com", password:"short", username:"Tedernator2" });
                expect(statusCode).toBe(400);
            })
            it("Returns 400 when password has spaces", async () => {
                const { statusCode } = await request(app).post("/users").send({  email:"teddyputus2.com", password:"short pass", username:"Tedernator2" });
                expect(statusCode).toBe(400);
            })
        }) 
    })
    ...
    describe("Log user out", () => {
        ...
        describe("With an invalid user", () => {
            it("returns 401 status if password is incorrect", async () => {
                ...
                const {statusCode, body} = await request(app).post("/users/logout").send({  email:"teddyputus2@gmail.com", password:"wrongpass"});
                expect(statusCode).toBe(401);
            })
            it("returns 404 status if user doesn't exist", async () => {
                const {statusCode, body} = await request(app).post("/users/logout").send({  email:"teddyputus69@gmail.com", password:"thisisapassword"});
                expect(statusCode).toBe(404);
            })
            it("returns 406 status if user already logged out", async () => {
                const {statusCode, body} = await request(app).post("/users/logout").send({  email:"teddyputus1@gmail.com", password:"thisisapassword"});
                expect(statusCode).toBe(406);
            })
    ...
    describe("Delete a user", () => {
        describe("With a valid user", () => {
            it("returns 200 status", async () => {
                const {statusCode} = await request(app).delete("/users").send({  email:"teddyputus1@gmail.com", password:"thisisapassword", username: "Tedernator"});
                expect(statusCode).toBe(200);
            })
        ...
        describe("With an invalid user", () => {
            it("returns 401 status if password is incorrect", async () => {
                await User.update({isLoggedIn : true}, {where:{  email:"teddyputus2@gmail.com"}})
                const {statusCode, body} = await request(app).delete("/users").send({  email:"teddyputus2@gmail.com", password:"wrongpass", username: "Tedworth"});
                expect(statusCode).toBe(401);
            })
            it("returns 404 status if user doesn't exist", async () => {
                const {statusCode, body} = await request(app).delete("/users").send({  email:"teddyputus1@gmail.com", password:"thisisapassword", username: "Tedernator"});
                expect(statusCode).toBe(404);
            })
        })
    })  
...
describe("Testing post routes", () => {
    ...
    describe("creating a post", () => {
        ...
        describe("With valid values", () => {
            ...
            it("returns 200 status", async () => {
                const { statusCode } = await request(app).post("/posts/1").send({  title:"testingtesting", content:"don't test me, fool!", isPrivate:true });
                expect(statusCode).toBe(200);
            })
            it("post exists in database", async () => {
                const post = await Post.findOne({where:{title: "testingtesting"}});
        ...
        describe("With invalid values", () => {
            ...
            it("returns 404 status with user that doesn't exist", async () => {
                const { statusCode } = await request(app).post("/posts/404").send({  title:"testingtesting", content:"don't test me, fool!", isPrivate:true });
                expect(statusCode).toBe(404);
            })
            it("returns 400 status when content too short", async () => {
                const { statusCode } = await request(app).post("/posts/1").send({  title:"testingtesting", content:"do", isPrivate:true });
                expect(statusCode).toBe(400);
            })
